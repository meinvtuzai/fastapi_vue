...
def Char(string: str, default=NONE, required=False, length=256, index=False):
    ...
    return Column(sql.String(length), default=default, server_default=str(text(default)), nullable=not required,
...
def Text(string: str, default=NONE, required=False, index=False):
    ...
    return Column(sql.Text, default=default, server_default=str(text(default)), nullable=not required,
                  comment=string, primary_key=primary_key)
