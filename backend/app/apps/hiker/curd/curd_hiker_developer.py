...
class CURDHikerDeveloper(CRUDBase):
    ...
    def setIsManager(self, db: Session, *, id: int, is_manager: bool, modifier_id: int = 0):
        return super().update(db, _id=id, obj_in={'is_manager': is_manager}, modifier_id=modifier_id)

    def setActive(self, db: Session, *, id: int, active: bool, modifier_id: int = 0):
        return super().update(db, _id=id, obj_in={'active': active}, modifier_id=modifier_id)
